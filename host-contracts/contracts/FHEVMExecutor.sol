...
contract FHEVMExecutor is UUPSUpgradeableEmptyProxy, Ownable2StepUpgradeable, FHEEvents {
    ...
    function verifyCiphertext(
        ...
        emit VerifyCiphertext(msg.sender, inputHandle, userAddress, inputProof, inputType, inputHandle);
        return inputHandle;
    }
    ...
    // 直接返回原 handle、原始类型字节和枚举值（均为编码后的 uint8）
    function peekTypeAndHandle(bytes32 handle)
        external
        pure
        returns (bytes32 outHandle, uint8 rawType, uint8 typeCt)
    {
        outHandle = handle;
        rawType = uint8(handle[30]);           // 你的格式里，type 在 [30]
        typeCt = uint8(FheType(rawType));      // 返回为 uint8，便于工具解析
    }
    ...
    function _binaryOp(
        ...
        //__Context_init_unchainedif (!acl.isAllowed(lhs, msg.sender)) revert ACLNotAllowed(lhs, msg.sender);
        if (scalar == 0x00) {
