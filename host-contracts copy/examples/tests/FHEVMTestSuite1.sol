...
contract FHEVMTestSuite1 {
    ...
    function encrypt_euint8() public {
        euint8 a = FHE.asEuint8(3);
        FHE.allowThis(a);
        euint8 b = FHE.asEuint8(4);
        FHE.allowThis(b);
    }

    event DebugAllowThis(bytes32 aProc, bytes32 bProc);

    function debug_external_euint8(externalEuint8 a, externalEuint8 b, bytes calldata inputProof) public {
        euint8 aProc = FHE.fromExternal(a, inputProof);
        euint8 bProc = FHE.fromExternal(b, inputProof);

        // 输出处理后的 aProc 和 bProc
        emit DebugAllowThis(euint8.unwrap(aProc), euint8.unwrap(bProc));
